# Projeto 3 - From Scratch - Prevendo o retorno financeiro de investimentos em títulos públicos

# Desenvolvimento do modelo de regressão sem o uso de Framework
...
# Para obter o caminho do diretório atual
path = getcwd()
...
def main():
	try:
        ...
        # Calcula os valores médios de x e y, covariância e variância	
		mean_x = mean(x_treino)
		mean_y = mean(y_treino)
		covar = covariance(x_treino, mean_x, y_treino, mean_y)
		var = variance(x_treino, mean_x)
		...
		print(f"Erro Médio do Modelo: {root_mean}\n")

		# Novos dados
		novo_x = float(input("Digite o Valor do Investimento: "))

		# Previsões
		novo_y = b0 + b1 * novo_x
		print(f"\nRetorno Previsto: {novo_y}")
		print("\n")
